...
class SampleTouchInteraction extends DisplayTouchInteraction{
  ...
  bool get _isPanning => _recentPanPosition.x.isNaN ||_recentPanPosition.y.isNaN;
  ...
  void _startPanFade(Point<num> speed) {
    ...
    _panFader = Timer.periodic(const Duration(milliseconds: 10), (timer) {
      ...
      _zoomPanel?.zoomController.panImageAbout(speed-Point<double>(dX*repetitions,dY*repetitions));
      repetitions++;
    });
  }
  ...
  void onScaleUpdate(TouchEvent event){
    ...
    _zoomPanel?.zoomController.scaleAbout(delta: scaleDelta.toInt(),mousePosition: event.client);
  }
