...
Point<num> rndPoint() => Point<num>(Random().nextInt(100), Random().nextInt(100));

void _genericPointerEventImageTranslationTest<TEvent extends PointerEvent>(TEvent evt) {
...
ZoomController _getFixedImagePosZoomControllerFake(Point<num> returnImgPos) {
 return  TestZoomController()..imagePosOffset = Offset(
     returnImgPos.x.toDouble(),returnImgPos.y.toDouble());
}
...
void main() {
    ...
    test('Triggering a pointer scroll event '
        '--> Should request the image transformation from the zoom controller',
            () => _genericPointerEventImageTranslationTest(FakePointerScrollEvent()));


    test('Triggering a pointer up event '
        '--> Should request the image transformation from the zoom controller',
