...
void main() {
  testWidgets(
      ...
      '--> Should register as listener on the supplied projector',
  ...
  testWidgets(
      'Pumping a CanvasWidget '
      '--> Should execute the projector during construction',
      (WidgetTester tester) async {
    final testProjector = TestProjector();
    await tester.pumpWidget(createCanvasWidget(projector: testProjector));
  ...
  testWidgets(
      'Pumping a CanvasWidget '
      '--> Should register as listener on the ZoomController',
      (WidgetTester tester) async {
    final testProjector = TestProjector();
    final testZoomController = TestZoomController();
    await tester.pumpWidget(createCanvasWidget(
        projector: testProjector, zoomController: testZoomController));
  ...
  testWidgets(
      'Pumping a CanvasWidget '
      '--> Should supply a overlay repaint callback on creation',
      (WidgetTester tester) async {
    final testInteractionController = TestInteractionController();
    await tester.pumpWidget(
        createCanvasWidget(interactionController: testInteractionController));
    expect(testInteractionController.setSelectionOverlayRepaintCallbackCalled,
        true);
  });
  ...
  testWidgets(
    ...
    await tester
        .pumpAndSettle(); // here we trigger "updateRenderObject" on the widget
    setter(() {});
  ...
  testWidgets(
      'Reassigning the projector '
      '--> listener should be removed from the old projector',
      (WidgetTester tester) async {
  ...
  testWidgets(
      ...
      '--> should trigger a relayout on the zoom controller',
      (WidgetTester tester) async {
    final testProjector = TestProjector();
    final testZoomController = TestZoomController();
    await tester.pumpWidget(
  ...
  testWidgets(
      ...
      '--> The projector should be called on simulated selection',
      (WidgetTester tester) async {
    final testProjector = TestProjector();
